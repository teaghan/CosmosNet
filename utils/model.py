...
def build_model(config, model_filename, device, build_optimizer=False):
    ...
    if build_optimizer:
        ...
        weight_decay = float(config['MIM TRAINING']['weight_decay'])
        init_lr = float(config['MIM TRAINING']['init_lr'])
        final_lr_factor = float(config['MIM TRAINING']['final_lr_factor'])
...
class MIM(timm.models.vision_transformer.VisionTransformer):
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        if self.mim_loss_fn=='mse':
            loss = torch.nn.functional.mse_loss(imgs, pred, reduction='none')
        elif self.mim_loss_fn == 'huber':
            loss = torch.nn.functional.huber_loss(pred, imgs, reduction='none', delta=1.0)
        else:
