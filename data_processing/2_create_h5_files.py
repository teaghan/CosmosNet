...
def create_h5_dataset(fits_paths, bands, use_calexp, img_size, out_dir, out_filename, scratch_dir='/scratch',
    ...
    for i, p in enumerate(getPatches(**kwargs)):
        ...
        p.match_catalogue(labels, size=img_size)
        if len(p.matches)>0:
            p.load()
            # Grab cutouts
            cutouts = get_cutouts_from_patch(p, bands=bands, size=img_size)        
            # Save to h5 file
...
if 'gal' in file_type:
    ...
    for out_name, use_calexp in zip(['HSC_dud_galaxy_GIRYZ7610_64'], #'HSC_dud_galaxy_calexp_GIRYZ7610_64',
                                    [False]): # True
        labels_path = os.path.join(out_dir,'HSC_galaxy.csv')
        patch_start = 0
        patch_end = 1515
...
if 'qso' in file_type:
    # QSOs
    for out_name, use_calexp in zip(['HSC_dud_qso_GIRYZ7610_224'],
                                    [False]):
...
if 'star' in file_type:
    # Stars
    for out_name, use_calexp in zip(['HSC_dud_star_GIRYZ7610_224'],
                                    [False]):
